...
The mockup hardcodes `CURRENT_PERIOD` as a "Minggu N · Bulan Tahun" string that resets at the start of each month. **That month-reset scheme was wrong and has been replaced (2026-09-07)** — it made the "current" period flip to a new one on the 1st of every month regardless of where the team actually was in their weekly reporting cadence, and it can't represent a week that spans a month boundary at all. `src/lib/period.ts`'s `currentPeriodLabel()`/`currentPeriodRange()` now implement the real rule, agreed between teams:
- Reporting weeks run **Minggu (Sun) → Sabtu (Sat)**, plain 7-day blocks, never reset by month.
- Teams get **Senin–Kamis of the following week** to log data for the week that just ended (an informal agreement between teams, not enforced/shown in the UI — no deadline banner or countdown).
- Because of that grace window, the "current" period for logging/tracking purposes is always the **most recently fully-completed** Sun–Sat week, not the week containing today. It doesn't roll forward to the in-progress week until that week itself finishes — e.g. on Senin 7 Sep 2026 (and through the following Sabtu 12 Sep), the current period is "30 Agu - 5 Sep 2026"; it only becomes "6 Sep - 12 Sep 2026" starting Minggu 13 Sep 2026.
- The label format is a plain date range, e.g. `"30 Agu - 5 Sep 2026"` — not `"Minggu N · Bulan Tahun"`. If you see that old format hardcoded or referenced anywhere, it's stale. A week crossing a year boundary prints the year on both ends, e.g. `"27 Des 2026 - 2 Jan 2027"` (`formatRange()` in `period.ts`).
- Wherever this period shows up in the UI it's labeled explicitly as **"Periode Pelaporan: ..."** (topbar, compose card, Riwayat entries) — added 2026-09-07 so it reads as an unambiguous reporting-period stamp rather than a bare date range.
- Compute it server-side from `now()` so "belum update minggu ini" is always evaluated against the real current period, not a stale constant. (Client components also call `currentPeriodLabel()` locally for display — fine since it's just formatting today's date the same way, not a source of truth.)
- **"Sudah/belum update" and the Riwayat display are both derived from `weekly_logs.submitted_at` via `isInCurrentPeriod()`/`currentPeriodLabel()`, never from a stored `period_label` string comparison** (changed 2026-09-07). The stored `period_label` column is still written at submission time for historical/Table-Editor reference, but nothing in the UI trusts it for matching or display anymore — that avoids "belum update" going permanently wrong for any row saved under an older label format (e.g. pre-2026-09-07 rows stored as `"Minggu 1 · September 2026"`), since date-range containment doesn't care what the label text looked like when it was written.
- **`isInCurrentPeriod()` compares period *identity*, not raw date containment — this was a real bug caught 2026-09-07, not just a hypothetical.** A log's `submitted_at` is filed during the grace window *after* the period it reports on, so it never literally falls inside that period's own Sun-Sat range — an earlier version of this function checked containment (`start <= submitted_at <= end`) and got it backwards both ways: an on-time grace-window submission (e.g. filed Sep 9 for period 30 Agu-5 Sep) showed as "belum update" for the period it was actually for, and then wrongly flipped to "sudah update" for the *next* period once that period became current on Sep 13 — because Sep 9 literally falls inside the *next* week's calendar days. The fix: run both the log's timestamp and "now" through `currentPeriodRange()` (which already implements "which period does this date belong to") and compare the two resulting `start` dates for equality, instead of checking whether the raw timestamp sits between `start` and `end`. If you ever touch this function again, re-verify against a grace-window submission (day+2 to day+4 after the period ends) — that's the case a naive containment check silently breaks.
